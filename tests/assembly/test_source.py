"""Tests for genome.assembly.source — what one assembly name resolves to, and in what order.
...
from genome.assembly import components as components_module
from genome.assembly import download as download_module
from genome.assembly import source as source_module
...
def test_the_seam_closes_the_download_annotation_chimera_cycle_in_one_direction_only() -> None:
    ...
    assert {name for name in reached if name.startswith("genome.annotation")} == set()
    ...
    assert "genome.assembly.components" in _module_level_imports(source_module)
    assert "genome.assembly.source" not in _module_level_imports(components_module)
    ...
    assert "genome.assembly.source" in _module_level_imports(download_module)
    assert "genome.assembly.chimera_build" not in _module_level_imports(download_module)
