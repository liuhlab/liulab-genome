"""Tests for genome.assembly.download.
...
reads or fakes the shipped TSV; the table itself is tested in test_metadata beside it.
"""
...
from genome.assembly.metadata import AssemblyMetadata, format_table_row
from genome.assembly.source import FetchedSource
from genome.store.checksum import ChecksumMismatchError, sha256_file
...
def test_downloading_an_assembly_imports_nothing_that_registers_an_annotation() -> None:
    ...
    # lives in `genome.annotation`, and neither imports the other. Were this edge to open,
    # the two would be one module by another route and the cycle `chimera_build` closes through
    ...
    forbidden = {"genome.assembly.genome"}
