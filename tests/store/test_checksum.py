"""Tests for genome.store.checksum — checksums and decompression, and nothing that runs a tool.
...
from genome.store import checksum
from genome.store.checksum import (
...
def test_gunzip_round_trips_and_the_layer_carries_no_run_of_its_own(tmp_path: Path) -> None:
    ...
    # in a module whose subject is checksums. `assembly/fasta` holds the tool now, so the
    # copy is gone; this keeps the layer from growing back.
    assert not hasattr(checksum, "_run")
    assert not hasattr(checksum, "_run_to")
