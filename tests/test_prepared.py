"""Tests for genome.io.prepared — the pipeline every **Prepared set** is prepared by.
...
from genome.io import prepared as prepared_mod
...
class TestEveryRootIsDeclaredInOnePlace:
...
def test_the_shared_pipeline_reaches_the_fetch_step_through_its_module() -> None:
    ...
    # `genome.io.fetch` must take every download in the package offline, which a module
    ...
    assert "from genome.io import fetch" in source_text
    assert "fetch.fetch_url(" in source_text
    assert "from genome.io.fetch import" not in source_text
