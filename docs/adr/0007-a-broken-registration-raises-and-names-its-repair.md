...
on disk — each naming the offending file and quoting `genome register <assembly> --force`, which is
also the repair: it keeps an unpacked FASTA whose sha256 is still the pinned one, and fetches again
